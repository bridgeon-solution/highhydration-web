...
export default function OrderManagement() {
  ...
  const fetchAllOrders = async () => {
    setLoading(true)
    try { 
      const response = await api.get(`/orders`, {params:{params:params,page,filter}});
  ...
  return (
    ...
    <div className='flex w-full h-screen overflow-hidden bg-[#F8F8F8]'>
      <div className='mt-2 min-h-screen'>
       ...
       {isOpen&&
    <FilterModalOrder  isOpen={isOpen} setIsOpen={setIsOpen} setFilter={setFilter}/>
    }
      <Box sx={{ width: '100%', overflow: 'scroll' }}>
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
            <Tab label="All" {...a11yProps(0)} onClick={()=>setParams('')}/>
            <Tab label="Bulk Orders" {...a11yProps(1)} onClick={()=>{setParams('bulkorder')}}/>
            <Tab label="Pre Orders" {...a11yProps(2)} onClick={()=>{setParams('preorder')}}/>
            <div className=' flex items-center p-2 px-5'>
        {!isOpen && <FaFilter size={16} onClick={()=>setIsOpen(true)} />}
      </div>
          </Tabs>
        </Box>
